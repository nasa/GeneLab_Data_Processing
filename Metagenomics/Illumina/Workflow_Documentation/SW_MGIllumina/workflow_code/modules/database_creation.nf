...
process SETUP_GTDBTK_DB {
    ...
    shell:
        ...
        # Make a copy of the download script to edit wget's timeout duration  
        cat `which download-db.sh` |sed 's/\$db_url/--timeout=3600 $db_url/' >  download-db.sh && \
        chmod +x ./download-db.sh
        # Downloading
        ./download-db.sh && touch GTDB-tk-ref-db/SETUP_GTDBTK_DB_SETUP
        printf "### Set up completed successfully ###\\n\\n"
        '''
}
...
workflow {
     make_databases(Channel.of(params.CAT_DB_LINK))
}
