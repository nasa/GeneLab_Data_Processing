...
if (organism %in% c("athaliana")) {
  ...
  unique_probe_ids <- retry_with_delay(
  ...
  if (!use_custom_annot) {
    ...
    # DEBUG:START
    if ( is.integer(params$DEBUG_limit_biomart_query) && params$DEBUG_limit_biomart_query > 0 && params$DEBUG_limit_biomart_query < length(probe_ids) ) {
    ...
    if (annot_type == 'agilent') {
      ...
      agilent_table <- read.delim(
        ...
        )[c('ProbeID', 'EnsemblID')] %>% dplyr::filter(!is.na(EnsemblID)) %>%
        dplyr::mutate(EnsemblID = stringr::str_split_i(EnsemblID, "\\.", 1))

      stopifnot(nrow(agilent_table) == length(unique(agilent_table$ProbeID)))

      # DEBUG:START
      if ( is.integer(params$DEBUG_limit_biomart_query) && params$DEBUG_limit_biomart_query > 0 && params$DEBUG_limit_biomart_query < nrow(agilent_table) ) {
        warning(paste("DEBUG MODE: Limiting query to", params$DEBUG_limit_biomart_query, "entries"))
        message(paste("DEBUG MODE: Limiting query to", params$DEBUG_limit_biomart_query, "entries"))
        transcript_ids <- agilent_table$EnsemblID[1:params$DEBUG_limit_biomart_query]
      } else {
        transcript_ids <- agilent_table$EnsemblID
      }
      # DEBUG:END
      ...
      #   Note: If timeout is occurring (possibly due to larger load on BioMart), reduce chunk size
      CHUNK_SIZE= 500
      transcript_id_chunks <- split(transcript_ids, ceiling(seq_along(transcript_ids) / CHUNK_SIZE))
      id_map <- data.frame()
      for (i in seq_along(transcript_id_chunks)) {
        ...
        message(glue::glue("Running BioMart query chunk {i} of {length(transcript_id_chunks)}. Total transcript IDS in query ({length(transcript_id_chunk)})"))
        chunk_results <- biomaRt::getBM(
            attributes = c(
...
# remove biomaRt from session info if not used for this dataset to avoid confusion in version reporting
if (organism %in% c("athaliana") || (use_custom_annot && annot_type != 'agilent')) {
...
if (organism %in% c("athaliana") || (use_custom_annot && annot_type != 'agilent')) {
  versions_buffer <- glue::glue_collapse(c(
    ...
    glue::glue("  homepage: https://bioconductor.org/packages/3.22/bioc/html/biomaRt.html"),
    glue::glue("  workflow task: PROCESS_AGILE1CH")
    ), sep = "\n")
}
