...
write.table(filtered_count_summary_tab, "../Filtered_Sequence_Data/ITS-filtered-read-counts.tsv", sep="\t", quote=F, row.names=F)
...
raw_and_trimmed_read_counts <- read.table("../Trimmed_Sequence_Data/ITS-trimmed-read-counts.tsv", header=T, sep="\t")

    # reading in filtered read counts
filtered_read_counts <- read.table("../Filtered_Sequence_Data/ITS-filtered-read-counts.tsv", header=T, sep="\t")
...
write_biom(biom_object, "../Final_Outputs/ITS-taxonomy-and-counts.biom")
    ...
    # making combined counts and taxonomy table
tax_and_count_tab <- merge(tax_tab, asv_tab)
write.table(tax_and_count_tab, "../Final_Outputs/ITS-taxonomy-and-counts.tsv", quote=FALSE, row.names=FALSE, sep="\t")
