##################################################################################
## R processing script for 16S-archaeal data of GLDS-126                        ##
...
filtered_out <- filterAndTrim(fwd=forward_trimmed_reads, forward_filtered_reads, reverse_trimmed_reads, reverse_filtered_reads, truncLen=c(100,100), maxN=0, maxEE=c(2,2), truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=TRUE)
...
write.table(count_summary_tab, "../Final_Outputs/16S-archaeal-read-count-tracking.tsv", sep = "\t", quote=F, row.names=F)
...
for (i in 1:dim(seqtab.nochim)[2]) {
  asv_headers[i] <- paste(">ASV_16S_archaeal", i, sep="_")
}

  # making and writing out a fasta of our final ASV seqs:
asv_fasta <- c(rbind(asv_headers, asv_seqs))
write(asv_fasta, "../Final_Outputs/16S-archaeal-ASVs.fasta")
...
write.table(asv_tab, "../Final_Outputs/16S-archaeal-counts.tsv", sep="\t", quote=F, row.names=FALSE)
...
write.table(tax_tab, "../Final_Outputs/16S-archaeal-taxonomy.tsv", sep = "\t", quote=F, row.names=FALSE)
