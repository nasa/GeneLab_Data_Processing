##################################################################################
## R processing script for 16S data of GLDS-146                                 ##
...
sample.names <- scan("unique-sample-IDs.txt", what="character")
  ...
  # setting variables holding the paths to the forward and reverse reads (this dataset already had the primers removed)
forward_reads <- paste0("../Raw_Data/", sample.names, "_R1_raw.fastq.gz")
...
filtered_out <- filterAndTrim(fwd=forward_reads, forward_filtered_reads, reverse_reads, reverse_filtered_reads, truncLen=c(140,130), maxN=0, maxEE=c(2,2), truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=TRUE)
