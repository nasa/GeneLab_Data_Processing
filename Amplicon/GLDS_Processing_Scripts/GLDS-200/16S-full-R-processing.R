...
write.table(filtered_count_summary_tab, "../Filtered_Reads/16S-filtered-read-counts.tsv", sep="\t", quote=F, row.names=F)
...
raw_and_trimmed_read_counts <- read.table("../Trimmed_Reads/16S-trimmed-read-counts.tsv", header=T, sep="\t")

    # reading in filtered read counts
filtered_read_counts <- read.table("../Filtered_Reads/16S-filtered-read-counts.tsv", header=T, sep="\t")
