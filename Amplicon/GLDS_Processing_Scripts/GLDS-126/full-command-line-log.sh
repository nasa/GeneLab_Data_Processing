...
rm ../Raw_Data/*fastqc*
  ...
  # running cutadapt to remove primers:
    # 16S-universal
for sample in $(cat 16S-universal-unique-sample-IDs.txt); do cutadapt -a ^GTGCCAGCMGCCGCGGTAA...ATTAGAWACCCBDGTAGTCC -A ^GGACTACHVGGGTWTCTAAT...TTACCGCGGCKGCTGGCAC -o ../Trimmed_Reads/${sample}-R1-primer-trimmed.fastq.gz -p ../Trimmed_Reads/${sample}-R2-primer-trimmed.fastq.gz ../Raw_Data/${sample}_R1_raw.fastq.gz ../Raw_Data/${sample}_R2_raw.fastq.gz --discard-untrimmed -m 100; done > ../Trimmed_Reads/16S-universal-cutadapt.log 2>&1
...
cat <( printf "sample\traw_reads\tcutadapt_trimmed\n" ) <( paste 16S-universal-unique-sample-IDs.txt <( grep "read pairs processed" ../Trimmed_Reads/16S-universal-cutadapt.log | tr -s " " "\t" | cut -f 5 | tr -d "," ) <( grep "Pairs written" ../Trimmed_Reads/16S-universal-cutadapt.log | tr -s " " "\t" | cut -f 5 | tr -d "," ) ) > ../Trimmed_Reads/16S-universal-trimmed-read-counts.tsv
    # 16S-archaeal
for sample in $(cat 16S-archaeal-unique-sample-IDs.txt); do cutadapt -a ^GYGCASCAGKCGMGAAW...CAGCMGCCGCGGTAA -A ^TTACCGCGGCKGCTG...WTTCKCGMCTGSTGCRC -o ../Trimmed_Reads/${sample}-R1-primer-trimmed.fastq.gz -p ../Trimmed_Reads/${sample}-R2-primer-trimmed.fastq.gz ../Raw_Data/${sample}_R1_raw.fastq.gz ../Raw_Data/${sample}_R2_raw.fastq.gz --discard-untrimmed -m 100; done > ../Trimmed_Reads/16S-archaeal-cutadapt.log 2>&1
      # keeping track of reads maintained
cat <( printf "sample\traw_reads\tcutadapt_trimmed\n" ) <( paste 16S-archaeal-unique-sample-IDs.txt <( grep "read pairs processed" ../Trimmed_Reads/16S-archaeal-cutadapt.log | tr -s " " "\t" | cut -f 5 | tr -d "," ) <( grep "Pairs written" ../Trimmed_Reads/16S-archaeal-cutadapt.log | tr -s " " "\t" | cut -f 5 | tr -d "," ) ) > ../Trimmed_Reads/16S-archaeal-trimmed-read-counts.tsv
