...
for sample in $(cat unique-sample-IDs.txt); do cutadapt -g CCTACGGGNGGCWGCAG -G GACTACHVGGGTATCTAATCC -o ../Trimmed_Reads/${sample}-R1-primer-trimmed.fastq.gz -p ../Trimmed_Reads/${sample}-R2-primer-trimmed.fastq.gz --discard-untrimmed -m 250 ../Raw_Data/${sample}_R1_raw.fastq.gz ../Raw_Data/${sample}_R2_raw.fastq.gz; done > ../Trimmed_Reads/cutadapt.log 2>&1

cat <( printf "sample\traw_reads\tcutadapt_trimmed\n" ) <( paste unique-sample-IDs.txt <( grep "read pairs processed" ../Trimmed_Reads/cutadapt.log | tr -s " " "\t" | cut -f 5 | tr -d "," ) <( grep "Pairs written" ../Trimmed_Reads/cutadapt.log | tr -s " " "\t" | cut -f 5 | tr -d "," ) ) > ../Trimmed_Reads/trimmed-read-counts.tsv
