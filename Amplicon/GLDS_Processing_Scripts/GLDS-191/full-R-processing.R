##################################################################################
## R processing script for 16S data of GLDS-191                                 ##
...
names(reverse_filtered_reads) <- sample.names
  ...
  # running filering step
    # reads are written to the files specified in the variables, the "filtered_out" object holds the summary results within R
      # there were dips in quality about half way through on most reads, but couldn't seem to work it out and retain a decent amount of reads, so moving forward 
filtered_out <- filterAndTrim(fwd=forward_trimmed_reads, forward_filtered_reads, reverse_trimmed_reads, reverse_filtered_reads, truncLen=c(250,200), maxN=0, maxEE=c(2,2), truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=TRUE)
...
raw_and_trimmed_read_counts <- read.table("../Trimmed_Reads/trimmed-read-counts.tsv", header=T, sep="\t")

    # reading in filtered read counts
filtered_read_counts <- read.table("../Filtered_Reads/filtered-read-counts.tsv", header=T, sep="\t")
