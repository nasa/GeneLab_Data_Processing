...
if (params$microbes) {
    ...
    counts_file <- list.files(params$input_gene_results_dir, pattern = "^FeatureCounts", full.names = TRUE)[1]
    
    if (!file.exists(counts_file)) {
        stop(paste("FeatureCounts file not found at:", counts_file))
    }
    ...
    txi.rsem <- list(counts = as.matrix(counts))
} else {
