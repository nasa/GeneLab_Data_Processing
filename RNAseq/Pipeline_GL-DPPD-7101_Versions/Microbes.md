...
This initial release of the Microbes pipeline documentation details the workflow steps executed when the `--microbes` parameter is used. In this mode, Bowtie2 is used for sequence alignment and FeatureCounts is used for gene-level quantification, replacing the default STAR and RSEM which are typically used for eukaryotic organisms.

Differences with default workflow:
- STAR is replaced with Bowtie2 for alignment
...
- rRNA genes are removed from FeatureCounts results on a dataset-wide basis. rRNA removal logs are all in the same file.
...
> **Note:** Steps 8a-8c are performed once as they set up the analysis framework. Steps 8d-8f are performed twice with different input files:
